...
static sqlite3* open_db(const std::string &path) {
    ...
    const char* schema =
        "CREATE TABLE IF NOT EXISTS signals (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT DEFAULT (datetime('now')), source TEXT, note TEXT);"
        "CREATE TABLE IF NOT EXISTS methods (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, params_json TEXT, created_at TEXT DEFAULT (datetime('now')));"
        "CREATE TABLE IF NOT EXISTS examples (id INTEGER PRIMARY KEY AUTOINCREMENT, signal_id INTEGER, method_id INTEGER, result TEXT, confidence REAL, notes TEXT, created_at TEXT DEFAULT (datetime('now')));";
    char* err = nullptr;
...
static void insert_example(sqlite3* db, int method_id, double confidence, const std::string &notes) {
    sqlite3_stmt* stmt = nullptr;
    const char* sql = "INSERT INTO examples (method_id, result, confidence, notes) VALUES (?, ?, ?, ?);";
...
static double attempt_gmsk_simple(const std::vector<std::complex<float>> &s, double min_power) {
    if (s.size() < 32) return 0.0;
    double sum_pow = 0.0;
    for (auto &c : s) sum_pow += std::norm(c);
...
static void async_write_snapshot(const std::string &dir, const std::vector<std::complex<float>> &samples, double conf, uint64_t ts_ns) {
    try {
        ...
        if (!samples.empty()) {
            ofs.write(reinterpret_cast<const char*>(samples.data()), samples.size() * sizeof(std::complex<float>));
        }
        ofs.close();
...
static void processing_thread_func(sqlite3* db, double min_power, double conf_threshold,
    ...
    while (running) {
        ...
        double sum_pow = 0.0;
        for (auto &c : block.samples) sum_pow += std::norm(c);
        ...
        if (conf > conf_threshold) {
            ...
            if (conf >= snapshot_conf) {
                ...
                std::thread t([dir_copy, samples_copy=std::move(samples_copy), conf_copy, ts_copy]() mutable {
...
static void capture_thread_func(double center_freq, double sample_rate, double gain, size_t block_len, long long read_timeout_us) {
    SoapySDR::Kwargs kw;
    ...
    while (running) {
        ...
        int ret = dev->readStream(rxStream, (void**)&buffs[0], (int)block_len, flags, ts, read_timeout_us);
        ...
        if (ret > 0) {
                                  ...
                                  std::chrono::steady_clock::now().time_since_epoch()).count();
            {
                ...
                while (q.size() > 64) q.pop_front(); // increase queue depth for adaptive mode
            }
            q_cv.notify_one();
        } else if (ret == 0) {
...
static long long env_to_ll(const char* name, long long def) {
...
static size_t env_to_sz(const char* name, size_t def) {
    const char* v = std::getenv(name);
    if (!v) return def;
    try { return std::stoul(v); } catch(...) { return def; }
}
...
int main(int argc, char** argv) {
    ...
    long long read_timeout_us = env_to_ll("RF_READ_TIMEOUT_US", env_to_ll("READ_TIMEOUT_US", 500000));
    ...
    std::cout << "Starting rf_adapt_intel: center=" << center_freq << " sps=" << sample_rate
              << " gain=" << gain << " block_len=" << block_len << " read_timeout_us=" << read_timeout_us
              << " conf_threshold=" << conf_threshold << " console_conf=" << console_conf
              << " snapshot_conf=" << snapshot_conf << " snapshot_dir=" << snapshot_dir << "\n";
    ...
    std::thread proc_th(processing_thread_func, db, min_power, conf_threshold, console_conf, snapshot_conf, snapshot_dir);
    std::thread cap_th(capture_thread_func, center_freq, sample_rate, gain, block_len, read_timeout_us);
    ...
    if (proc_th.joinable()) proc_th.join();
    sqlite3_close(db);
    return 0;
}